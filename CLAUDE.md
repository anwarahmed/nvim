...
**This branch (`nvim-omarchy`) targets Omarchy on Arch Linux only** — do not add macOS or other-platform support to it. `main` is the portable branch: it carries a standalone `lua/anwar/plugins/colorscheme.lua`, which this branch deletes and replaces with the Omarchy theme pipeline described below. That swap is the entire difference between the two branches, so keep Omarchy-specific work here and port nothing back to `main` unless it is platform-neutral.
...
This is the most involved part of the config and spans several files plus state outside the repo.
...
- Work on a feature branch and open a PR against `main`; history shows every change landing as a merged PR.
